...
export async function GET() {
  ...
  const doc = {
    ...
    info: {
      ...
      version: "1.0.0",
      description:
        "On-chain token safety check for EVM tokens (Ethereum, BNB Chain, Base). Give a token address and chain, get a GO / CAUTION / DANGER verdict read directly from chain via RPC: honeypot/sellability simulation, buy/sell tax, holder concentration, deployer privileges, upgradeable-proxy, ownership renounce, mint capability, and LP burn, with scoring that adapts to liquidity depth. Pay-per-call in USDC via the x402 protocol on Base. No API key, no signup.",
      "x-guidance":
